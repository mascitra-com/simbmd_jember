...
class Asset_c extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model('admin/Kibc_model', 'kib');
	...
	public function index()
	{
		...
		$this->render('modules/admin/asset_c/index', $data);
	}
	...
	public function add()
	{
		...
		$data['skpds'] = $this->skpd->order_by('name')->get_all();
		$this->render('modules/admin/asset_c/import', $data);
	}
	...
	public function add_rehab($id = null)
	{
		...
		$this->render('modules/admin/asset_c/rehab', $data);
	}
	...
	public function insert_rehab()
	{
		...
		$sukses = $this->kib->insert($data);
		if($sukses) {
			$this->message('Rehab berhasil ditambah','success');
			$this->go('admin/asset_c');
		} else {
			$this->message('Rehab gagal disimpan','danger');
			$this->go('admin/asset_c/add_rehab/'.$data['asset_id']);
		}
	}
	...
	public function edit($id = null)
	{
		...
		$data['categories'] = $this->category->order_by('nama')->get_all();
		$this->render('modules/admin/asset_c/form', $data);
	}
	...
	public function delete($id = null)
	{
		...
		if($sukses) {
			$sukses = $this->kib->update_by(array('rehab_to'=>$id), array('status'=>'ON_DELETE'));
			if($sukses) {
				$this->message('Data berhasil dihapus dan <b>menunggu persetujuan BPKA.</b>','success');
				$this->go('admin/asset_c');
			} else {
				$this->kib->update($id, array('status'=>''));
