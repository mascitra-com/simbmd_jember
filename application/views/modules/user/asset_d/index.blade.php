	...
	<div class="col">
		<div class="card">
			<div class="card-header form-inline">
				<!-- BATCH ACTION -->
				<form action="{{site_url('user/asset_d/batch_action')}}" method="POST" id="form-batch" class="hidden-sm-down">
					{{$csrf}}
				...
				<!-- ADD BUTTON -->
				<a href="{{site_url('user/asset_d/add')}}" class="btn btn-primary"><i class="fa fa-plus mr-2"></i>Tambah / Import</a>
			</div>
					...
					<table class="table table-striped table-hover table-bordered">
						<thead>
							<tr>
								...
								<th class="text-nowrap">Tgl. Pengadaan</th>
								...
								<th class="text-nowrap">Nomor Tanah</th>
							...
							<!-- FILTER DATA  -->
							<form action="{{site_url('user/asset_d')}}" method="GET">
								<tr>
									...
									<th class="th-form"><input type="text" name="kontruksi" class="form-control" value="{{isset($filter['kontruksi']) ? $filter['kontruksi'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="text" name="panjang" class="form-control" value="{{isset($filter['panjang']) ? $filter['panjang'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="text" name="lebar" class="form-control" value="{{isset($filter['lebar']) ? $filter['lebar'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="number" name="luas" class="form-control" value="{{isset($filter['luas']) ? $filter['luas'] : ''}}" placeholder="-"></th>
									...
									<th class="th-form"><input type="text" name="nomor_tanah" class="form-control" value="{{isset($filter['nomor_tanah']) ? $filter['nomor_tanah'] : ''}}" placeholder="-"></th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-nowrap">
									<div class="btn-group btn-group-sm" role="group" aria-label="...">
										<a href="{{site_url('user/asset_d/edit/'.$kib->id)}}" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
										<a href="{{site_url('user/asset_d/delete/'.$kib->id)}}" class="btn btn-danger" onclick="return confirm('Apakah anda yakin?')"><i class="fa fa-trash"></i></a>
										<a href="{{site_url('user/asset_d/add_rehab/'.$kib->id)}}" class="btn btn-primary"><i class="fa fa-plus mr-2"></i><i class="fa fa-home"></i></a>
										<button class="btn btn-secondary btn-mutasi" data-id="{{$kib->id}}"><i class="fa fa-exchange mr-2"></i>Mutasi</button>
								...
								<td class="text-nowrap">{{$kib->kontruksi}}</td>
								<td class="text-nowrap">{{$kib->panjang}}</td>
								<td class="text-nowrap">{{$kib->lebar}}</td>
								...
								<td class="text-nowrap">{{$kib->nomor_tanah}}</td>
							...
							<tr class="hide rehab{{$kib->id}}">
								...
								<td class="text-center">
									<a href="{{site_url('user/asset_d/edit/'.$rehab->id)}}" class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i></a>
									<a href="{{site_url('user/asset_d/delete/'.$rehab->id)}}" class="btn btn-sm btn-danger" onclick="return confirm('Apakah anda yakin?')"><i class="fa fa-trash"></i></a>
								</td>
								...
								<td class="text-nowrap">{{$rehab->kontruksi}}</td>
								<td class="text-nowrap">{{$rehab->panjang}}</td>
								<td class="text-nowrap">{{$rehab->lebar}}</td>
								<td class="text-nowrap">{{$rehab->luas}}</td>
								...
								<td class="text-nowrap">{{$rehab->nomor_tanah}}</td>
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{site_url('user/asset_d/exchange')}}" method="POST">
					{{$csrf}}
