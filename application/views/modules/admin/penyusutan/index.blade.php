	...
	<div class="col">
		<div class="card">
			...
			<div class="card-block">
				<form action="{{site_url('admin/penyusutan/generate')}}" method="GET">
					<!-- {{$csrf}} -->
					<div class="form-group row">
						<label class="col-md-2 col-form-label">Pilih OPD</label>
						<div class="col-md-4">
							<select name="skpd_id" class="form-control">
								<option value="">Pilih OPD. . .</option>
								@foreach($skpds AS $skpd)
								<option value="{{$skpd->id}}">{{strtolower($skpd->name)}}</option>
								@endforeach
							</select>
						</div>
					</div>
