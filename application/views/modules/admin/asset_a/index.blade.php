@layout('common/main')
@section('title')Aset KIB-A@endsection
	...
	<div class="col">
		<div class="card">
			<div class="card-header form-inline">
				<!-- BATCH ACTION -->
				<form action="{{site_url('admin/asset_a/batch_action')}}" method="POST" id="form-batch" class="hidden-sm-down">
					{{$csrf}}
				...
				<!-- ADD BUTTON -->
				<a href="{{site_url('admin/asset_a/add')}}" class="btn btn-primary"><i class="fa fa-plus mr-2"></i>Tambah / Import</a>
			</div>
				...
				<div class="table-container">
					<table class="table table-striped table-hover table-bordered">
						<thead>
							...
							<!-- FILTER DATA  -->
							<form action="{{site_url('admin/asset_a')}}" method="GET">
								<tr>
									...
									<th class="th-form"><input type="text" name="hak" class="form-control" value="{{isset($filter['hak']) ? $filter['hak'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="text" name="tanggal_sertifikat" class="form-control" value="{{isset($filter['tanggal_sertifikat']) ? $filter['tanggal_sertifikat'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="text" name="nomor_sertifikat" class="form-control" value="{{isset($filter['nomor_sertifikat']) ? $filter['nomor_sertifikat'] : ''}}" placeholder="-"></th>
									<th class="th-form"><input type="text" name="pengguna" class="form-control" value="{{isset($filter['pengguna']) ? $filter['pengguna'] : ''}}" placeholder="-"></th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-nowrap">
									<div class="btn-group">
										<a href="{{site_url('admin/asset_a/edit/'.$kib->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-pencil"></i></a>
										<a href="{{site_url('admin/asset_a/delete/'.$kib->id)}}" class="btn btn-sm btn-warning" onclick="return confirm('Apakah anda yakin?')"><i class="fa fa-trash"></i></a>
								...
								<td class="text-nowrap">{{$kib->hak}}</td>
								<td class="text-nowrap">{{$kib->tanggal_sertifikat}}</td>
								<td class="text-nowrap">{{$kib->nomor_sertifikat}}</td>
								<td class="text-nowrap">{{$kib->pengguna}}</td>
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{site_url('admin/asset_a/exchange')}}" method="POST">
					{{$csrf}}
